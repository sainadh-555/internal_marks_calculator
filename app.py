...
with tab1:
    ...
    with col3:
        pre_t1_1_m2 = st.number_input("Pre-T1-1 Marks", step=1, key="m2_pre_t1_1")
        t1_m2 = st.number_input("T1 (MID) Marks", step=1, key="m2_t1")
        t3 = st.number_input("T3 Marks (Combined T3-1 & T3-2 out of 10)", step=1, key="m2_t3")
    with col4:
        pre_t1_2_m2 = st.number_input("Pre-T1-2 Marks", step=1, key="m2_pre_t1_2")
        t2 = st.number_input("T2 Marks (Combined T2-1 & T2-2 out of 10)", step=1, key="m2_t2")
        t4 = st.number_input("T4 Marks (out of 40)", step=1, key="m2_t4")
...
with tab2:
    ...
    if st.button("Calculate Practice Marks", type="primary", use_container_width=True):
        try:
            ...
            # Calculations
            module_1 = ass_m1_avg * (25 / max_marks_m1)
            module_2 = ass_m2_avg * (35 / max_marks_m2)
